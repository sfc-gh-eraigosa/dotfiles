...
| :-- | :-- | :-- |
| 2026-08-16 | update --ref | Diagnosed a reported "status doesn't reflect update": correct behaviour — the install-stamp step ships in THIS PR, not on `main`, so `update`-to-main writes no stamp (nano shows a stamp only because it's on the feature branch). `update` hardcoded `main` with no target override, so there was no way to prove the stamp pre-merge. Added `update --ref` (default `main`) + `validRef` shell-injection guard; made `recordingRunner` capture the interactive command too. TDD; live smoke: `--help` shows `--ref`, injection ref rejected before any host is touched. Gate: 74%. |

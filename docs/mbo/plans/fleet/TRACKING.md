...
| 5 — stamp parser | drift | **done** | (this commit) | `go test ./internal/stamp/ -cover` → ok, **100.0%**, 6 tests → `evidence/task05/stamp.txt` | Strict on commit length + installed_at so a truncated write can't masquerade as a valid install; unknown keys ignored for forward compat. |
| 6 — drift classify + age | drift | **done** | (this commit) | `go test ./internal/drift/ -cover` → ok, **100.0%**, 6 tests → `evidence/task06/drift.txt`; `grep time.Now()` in non-comment code → 0 | Clock injected (verified). Added two tests beyond the plan: unreachable dominates every other signal, and a zero install time renders `-` rather than a fabricated duration. |

...
- [x] Leaf B: tasks 6–16 done; `go test -race -cover ./internal/updexec ./internal/runner` — updexec ≥ 90 % (observed: 92.7 % at gate, 92.5 % after three review rounds); moved invariant tests green; mux invariant green → `evidence/updexec/leaf-gate.txt`
...
- [x] Leaf E: tasks 24–26 done; every `tui_*` test green; live TUI transcript → `evidence/tui/` (G5 itself pending on the operator — no tty in the build session)
...
| 2026-09-05 | build-6 | Final D+E review (`/code-review 4e251ae..HEAD medium`): 10 findings fixed in `bcbd5e2` (TUI preamble join was a shell syntax error; Background lane runs `interactive: true` run steps as batch when the TUI holds a sudo credential — the prior unattended semantics; extras loop gated by the `&&` chain; `--json` exits non-zero on failure; typed `NeedsTerminal`; `--update-ref` no longer defaults to main; executor tees remote output into the capture; unbounded UI line queue; handoff forwards `--repo`; rune-safe plan label). De-flaked the fake I/O matcher; stub sleeper `exec`'d (31 s → 2.3 s); four staticcheck hits cleared (`46ffb03`). Final gate: `go test -race -cover ./...` green (cmd 63.3 %, updexec 92.5 %, updplan 93.9 %, featflag 96.6 %), `golangci-lint` clean, `gff lint` clean, built binary dry-run against a live `#fleet` host matches today's script byte for byte. PR #270 promoted to ready for review. Pending on the operator: G2-live, G3, G5–G9 (mutating live runs); follow-up: `gff get` shell-out to shed the +5.5 MB SDK link. |

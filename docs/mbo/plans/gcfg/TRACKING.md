...
| P0-T4 manifest flow + CLI | in-progress | (commit pending) | automated: `go test ./... -count=1 -cover` → main 90.9% · cmd 88.5% · pkg 79.4% · version 100%; module total 86.7%; `-race` x3 ok; leak grep clean; 21 cmd+pkg tests (manifest form fields/org/hook, expired code → 502 + clear error, wrong state → 400 ignored, port fallback, opener error, cancel; create/install/token/status/doctor incl. token stdout = value only) → [evidence/P0-T4/2026-09-05-automated.txt](./evidence/P0-T4/2026-09-05-automated.txt) | human: one real create + token mint → `evidence/ghapp/` pending |

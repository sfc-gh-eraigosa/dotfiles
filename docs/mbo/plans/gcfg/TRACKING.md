...
| P1-T1 gcfg scaffold + CI | done | `f160feb` | `go test ./... -count=1 -cover` → main 91.7% · cmd 100% · version 100%; module total 96.7% ≥80; `go run . version`; actionlint clean → [evidence/P1-T1/2026-09-05.txt](./evidence/P1-T1/2026-09-05.txt) | gcfg-ci gates 80/90/90; schema-drift + actionlint + leak-grep steps skip themselves until P3/P5 produce their inputs |
| P1-T2 schema load + lint + JSON Schema | done | `4aad213` | `go test ./internal/schema/ -count=1 -cover` → **94.8%** ≥90; cmd 96.9%; module total 89.2% ≥80. Compiled binary: clean file → exit 0; 4 problems (enum, label colour, duplicate name, org block outside `.github`) → exit 2; unknown key → `line 5: field squash_titel not found` exit 2; secret-shaped value refused **without echoing it**; empty file warns at exit 0; `gcfg schema` byte-stable across runs → [evidence/P1-T2/2026-09-05.txt](./evidence/P1-T2/2026-09-05.txt) | List[T] accepts both the bare-list and `{ownership, items}` forms and round-trips to the simpler one; lint order is schema order, not YAML order |

...
| T7 model → model page (review follow-up) | done | 32d819d | `TestModelFamily`/`TestModelURL`/`TestParse_ModelID`/`TestAI_Spans_ModelURL_*` pass; linked goldens show anthropic.com/claude/opus (evidence/T7-T8/) | family map from model.id; unknown family → no model link |
| T8 directory → vscode.dev view (review follow-up) | done | 32d819d | `TestVSCodeDevURLs`/`TestDirGit_DirSpan_*`/`TestRepo_UsesPrecomputedPR` pass; live: this worktree dir → vscode.dev/…/pull/279/changes | PR pre-looked-up once in cmd (`Deps.PR`), shared by both segments |
...
| 2026-09-05 | build-2 | Review follow-ups: T7 model → model page, T8 directory → vscode.dev PR/branch view; installed for the user's check. |
| 2026-09-05 | build-2 | Human check: directory → vscode.dev …/pull/279/changes and model → anthropic.com/claude/fable both open (user: "Both open the expected pages"). CI on the merge push: all checks pass. |

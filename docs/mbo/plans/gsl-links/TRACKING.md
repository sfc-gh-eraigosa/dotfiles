...
| T7 model → model page (review follow-up) | done | (this commit) | `TestModelFamily`/`TestModelURL`/`TestParse_ModelID`/`TestAI_Spans_ModelURL_*` pass; linked goldens show anthropic.com/claude/opus (evidence/T7-T8/) | family map from model.id; unknown family → no model link |
| T8 directory → vscode.dev view (review follow-up) | done | (this commit) | `TestVSCodeDevURLs`/`TestDirGit_DirSpan_*`/`TestRepo_UsesPrecomputedPR` pass; live: this worktree dir → vscode.dev/…/pull/279/changes | PR pre-looked-up once in cmd (`Deps.PR`), shared by both segments |

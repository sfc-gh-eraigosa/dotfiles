...
| :-- | :-- | :-- | :-- | :-- |
| 2026-09-06 | T1 (contract, **frozen**) | `provider.ReservedKeys` was written from the design doc, not from fleet's running keymap, and drifted: **six keys fleet binds live were left free for a provider to shadow.** The `fleet-connect-k8s` design had already spent `l` on logs. | A test over fleet's `keyHelp` vs `ReservedKeys`: `fleet binds "l" (show / hide the streaming log pane) but a provider may declare it`; likewise `s` (ssh to cursor host), `?`, `J`, `K`, `e`. Confirmed separately that `Action{Key:"l"}.Validate()` returned nil. | **Amendment applied in PR #305 before merge** (the freeze protects downstream leaves from quiet edits; this is an explicit, reviewed change to the frozen contract, recorded here and in design §4.10). `ReservedKeys` now mirrors fleet's real keymap plus the held nav keys `h` and `:`; `TestEveryFleetKeyIsReservedAgainstProviders` makes the mirror mechanical, and a second test refuses an unjustified reservation. `FakeProvider` and the k8s design moved off `l` (→ `o`) and `e` (→ `E`). |
...
| 2026-09-06 | cross-cutting review (`sdk/libs/tui`) | The operator asked whether `ReservedKeys` collides with the shared TUI keymap that landed 2026-09-05, ahead of the `fleet-tui` adoption and PR #304 — adopt, do not block. The `/code-review` agent could not answer it: it inherited the session's working directory (the *playground* repo) and correctly refused to invent findings for a `sdk/fleet` that does not exist there. Reviewed directly instead. **Finding was worse than the shared-lib question:** the contract did not match *fleet's own* keymap (six live keys unreserved). Fixed per design §4.10 — the mirror is now test-enforced, `h` and `:` are held ahead of use, and nothing from `sdk/libs/tui` is adopted here. The library's `h`/`l` page motions would break fleet's shipped `l`; its guide's `Without` escape is the likely resolution and belongs to `fleet-tui`. |

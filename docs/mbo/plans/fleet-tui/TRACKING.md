# fleet-tui — live state ledger

- **Slug:** fleet-tui
- **Started:** 2026-08-16
- **Playbook:** [`IMPLEMENTATION.md`](./IMPLEMENTATION.md) · **Cursor:** [`TODO.md`](./TODO.md)
- **Plan (source of truth):** [`../fleet-tui.md`](../fleet-tui.md) · spec [`../../specs/fleet-tui.md`](../../specs/fleet-tui.md)

> **Update after EVERY task.** Status: `todo · in-progress · blocked · done`.
> A row is `done` only with a commit SHA **and** observed evidence.
> **Never write a result you did not observe.**

## 0. Worker registry

| Worker | Worker ref | Branch | PR | State |
| :-- | :-- | :-- | :-- | :-- |
| design (this) | `fleet-tui/<user>/mbo` | `feature/fleet-tui/<user>/mbo` | [#227](https://github.com/sfc-gh-eraigosa/dotfiles/pull/227) | active |
| build | `fleet-tui/<user>/build` | `feature/fleet-tui/<user>/build` | TBD | not created |

## 1. Task ledger

| Task | Status | Commit | Evidence (command → result) | Notes |
| :-- | :-- | :-- | :-- | :-- |
| 1 — mechanical split + alias cursor | **done** | see log | `go build` OK; tui.go 55 lines; TestRowArrivalResortsButKeepsCursorOnItsAlias | zero behavior change; tui.go < 60 lines |
| 2 — theme + styled frames (F3) | **done** | see log | 11 demo frames render + fit width (`FLEET_DEMO=1 go test ./cmd/ -run TestDemoFrames -v`) | Ascii-profile goldens |
| 3 — viewport + resize (F4c,d) | **done** | see log | TestWindowResizeKeepsCursorVisible, TestHalfPageMovesAndStaysInBounds | clampViewport invariant |
| 4 — vim motions (F4a,b) | **done** | see log | TestGGJumpsTopAndGJumpsBottom, TestLoneGIsCancelledByTheNextKey | gg pending-key state |
| 5 — streaming collect + spinner (F1) | **done** | see log | probeHost extracted; headless status tests unchanged-green | collectOne extraction; headless frozen |
| 6 — refresh `r` (F2) | **done** | see log | TestRefreshRepollsKeepingCursorAndSelection | v1's dead key becomes real |
| 7 — `/` search (F5) | **done** | see log | smartcase + invalid-regex + mode-swallow tests | smartcase; invalid-regex UX |
| 8 — `n`/`N` (F6) | **done** | see log | TestNextPrevMatchWrapsBothWays | wraparound both ways |
| 9 — selection + visual (F7) | **done** | see log | TestSpaceTogglesSelectionByAliasAcrossResort | alias-keyed; survives refresh |
| 10 — concurrent bg update engine (F8,F9,F2b) | **done** | see log | TestBackgroundWaveNeverExceedsJobLimit (running<=jobs at every step), FailedUpdateAdvances, RefreshSkipsUpdating | ≤ jobs slots; BatchMode fast-FAIL; TUI stays live; refresh excludes updating |
| 11 — interactive fallback queue (F13) | **done** | see log | TestPrecheckRoutes..., TestInteractiveFallbackRunsOnlyAfterTheWaveDrains | sudo-precheck routing; serial handoffs after the wave |
| 12 — `s` ssh action (F10) | **done** | see log | TestSSHIsBlockedWhileThatHostUpdates | argv exactly `ssh <alias>`; blocked while updating |
| 13 — help overlay + polish (F11,F12) | **done** | see log | overlay from keyHelp; TestQuitIsGuardedWhileUpdatesRun | overlay from the keymap table; quit guard |
| 14 — docs + gate | **done** | see log | `scripts/test.sh` -> OK: coverage for fleet is 80% (min 60); go vet clean; 67 tests | fleet ≥60%, cmd ≥55%, vet, leak sweep |
| 16 — unattended answers (sudo + prompt pre-answers) | **done** | see log | winsetup_ask/gemini_teardown env contracts EXECUTED (y/n/s + skip/bogus/flag-wins); 12 demo frames; secret-never-in-argv + stdin-only + prime-verified tests |
| 17 — `discover --add-all` bulk adopt | **done** | see log | live: dry-run + decline write nothing; --yes adopts 2; directives/operator comment/pattern hosts preserved; adopt-all→remove byte-identical; second run = no write, still 1 backup |
| 15 — HUMAN STOP: live capture | todo | | | tmux; 2 concurrent updates + navigation; sudo prompt on fallback |
| 18 — `RunVia` seam (F14) | **done** | pending commit | `go test ./internal/runner/...` OK; TestViaArgsRelaysThroughPeerWithProxyJump, TestFakeRecordsTheRelayHop | `ssh -J`; target is the ssh host, peer is the hop |
| 19 — wake ladder `internal/reach` (F14a–g) | **done** | pending commit | `go test ./internal/reach/... -cover` OK, **82.4%**; 14 tests incl. TestRelaySuccessAloneNeverReportsWoke | zero sockets, zero real elapsed time |
| 20 — auto-wake in probeHost (F15, F18a) | **done** | pending commit | TestWokenHostReportsItsRealClassAndHowItWoke, TestSleepingHostsWakeConcurrentlyNotSerially (peak>1, elapsed<N×budget) | existing status tests untouched; nil waker = old behaviour |
| 21 — `fleet wake` verb (F16, F18b) | **done** | pending commit | `evidence/task19/wake-verb.txt` — unit + **live** run: ladder escalated to peer-relay, exit 1, no false green | non-mutation argv allowlist pinned |
| 22 — TUI `w` + `waking` ownership (F17) | **done** | pending commit | `evidence/task20/tui-wake.txt`; TestRefreshSkipsHostsBeingWoken, TestHelpListsTheWakeKey | background lane only; never ExecProcess |
| 23 — docs + gate | **done** | pending commit | `evidence/task21/gate.txt` — vet clean, gofmt clean, fleet **81%** (min 60), portability Tier1/2 = 0, markdownlint at baseline | README wake section + 5 new AGENTS invariants |
| 24 — HUMAN STOP: hardware wake reproduction | **todo** | — | needs an elevated ARP flush (or a naturally cold cache) to recreate the sleeping-host state | see session log |
| 25 — probe carries the live branch | **done** | pending commit | `evidence/task25/probe-branch.txt`; TestBranchCostsNoExtraRoundTrip proves ONE dial | stamp half untouched; older hosts (no delimiter) still classify |
| 26 — branch column | **done** | pending commit | `evidence/task26/branch-column.txt`; demo frame shows `feature/gff≠main` beside `divergent` | found + fixed a pre-existing bug: alias was never truncated, so a long hostname overflowed any width |
| 27 — sticky answers | **done** | pending commit | `evidence/task27/sticky.txt`; the two inverted tests REPLACED, not deleted | deliberate invariant reversal, recorded in design §4.2 |
| 28 — confirm summary, `e`, `F` | **done** | pending commit | `evidence/task28/confirm-summary.txt`; secret never legible in any frame | the compensating control for a session-lived credential |
| 29 — answers persistence | **done** | pending commit | `evidence/task29/persistence.txt`; secret absent from marshalled bytes; hostile file ignored | caught a hermeticity bug: the model wrote to the developer's REAL ~/.config — path is injected now |
| 30 — select-all + docs + gate | **done** | pending commit | `evidence/task30/gate.txt` — vet/gofmt clean, fleet **82%**, portability 0/0, markdownlint at baseline | `a` respects the active `/` filter |

## 2. Feature → proof matrix (spec §5)

| Feature | Automated proof | Live proof (Task 14) |
| :-- | :-- | :-- |
| F1 streaming | [ ] F1a/F1b/F1c tests | [ ] rows arrive independently |
| F2 refresh | [ ] F2a · [ ] F2b (during updates) | [ ] r re-polls in place mid-wave |
| F3 theme | [ ] F3a goldens | [ ] colors on a real terminal |
| F4 motion+paging | [ ] F4a–d | [ ] big-fleet paging |
| F5 search | [ ] F5a–c | [ ] /regex highlight |
| F6 n/N | [ ] F6a | [ ] |
| F7 selection | [ ] F7a–b | [ ] visual range |
| F8 concurrent bg update | [ ] F8a–f | [ ] 2 hosts updating at once, TUI live |
| F9 --update-ref/--jobs | [ ] F9a–b | [ ] optional |
| F10 ssh action | [ ] F10a | [ ] in and back out |
| F11 help | [ ] F11a | [ ] |
| F12 empty fleet | [ ] F12a | — |
| F13 interactive fallback | [ ] F13a–b | [ ] sudo prompt reaches operator |

## 3. Done-when — the stop condition

- [ ] Tasks 1–15 `done` with SHAs + evidence
- [ ] `scripts/test.sh` green; fleet ≥ 60%, cmd ≥ 55%
- [ ] `go vet ./...` clean in sdk/fleet
- [ ] Headless status/update tests unmodified (except Task 5's addition)
- [ ] Live capture committed (Task 15) — incl. concurrent updates + mid-wave refresh
- [ ] README/AGENTS updated; no identity leak
- [ ] `docs/mbo/index.md` → `in-review`

## 4. Blockers & escalations

| Date | Task | Blocker | Command + observed output | Resolution |
| :-- | :-- | :-- | :-- | :-- |
| | | *(none yet)* | | |

**Pre-identified risks:**

| Risk | Where it bites | What to do |
| :-- | :-- | :-- |
| Stale local main under a new worktree (hit 3× on fleet) | worker creation | preflight §2.1; reset to origin/main |
| Golden frames vary across lipgloss/termenv versions | Tasks 2+ | pin Ascii profile in TestMain; goldens assert the Ascii frame only |
| ExecProcess error paths differ by tea version | Tasks 11–12 | F13b test drives via messages, not a real terminal |
| Spinner ticks make frames nondeterministic | Task 5 | fixed spinner frame injected in tests (no Tick in test path) |
| `sudo -n` passes but a later prompt appears (cred-cache expiry mid-run) | Task 10 | BatchMode=yes turns any prompt into a fast FAIL with the cause in the row log; operator re-runs via fallback |
| A background update and a refresh poll race on one host | Tasks 6/10 | in-flight ownership invariant (one of pending/updating/resolved); F2b test pins it |

## 5. Session log (append-only)

| Date | Session | What advanced |
| :-- | :-- | :-- |
| 2026-08-17 | sticky-answers + branch | Operator: answers were lost on esc/selection-change so a fleet-wide update could not be applied consistently; and there was no way to see which branch a host is on before targeting it. Reversed the "form starts empty every wave" invariant — answers now persist for the session, `u` skips to a confirm strip that DISPLAYS them (masked), `e` edits, `F` forgets, and only the non-secret half persists to `~/.config/fleet/answers.json` (0600, and the on-disk type has no credential field, so the leak is unrepresentable). Added `a` select-all respecting the active `/` filter. Branch: the stamp already parsed `branch` and probeHost discarded it; the LIVE branch now rides in the SAME round-trip as the stamp read (delimiter-split, older hosts still classify), rendering `feature/x≠main` when they differ — which explains an otherwise mysterious `ahead/divergent` row — and joins the search haystack so `/feature` → `a` → `u` is one workflow. Three real bugs found by tests rather than review: the alias column was never truncated (long hostnames overflowed at ANY width), `failWidth` budgeted from a hardcoded prefix plus a floor that pushed past the edge, and `newTUIModel` resolved the config path itself so every test wrote into the developer's real `~/.config/fleet`. Tasks 25–30 done; fleet coverage 80% → 82%. |
| 2026-08-17 | wake | Operator hit a real incident: a Pi was unreachable from the WSL2 workstation for hours while a peer on the SAME AP/band/channel reached it in 6 ms. Diagnosed to Wi-Fi `power_save on` (DTIM 3) — the radio sleeps through the BROADCAST ARP a cold neighbour cache must send, so the workstation gave up at layer 2 before any SSH. Generalised the manual fix (`ssh -J <peer> <target>` + make the target transmit at us) into `internal/reach`: a bounded ladder retry → local-prime → peer-relay, auto-run whenever a probe would say `unreachable`, plus a `fleet wake` verb, a TUI `w` key, and `--no-wake`/`--wake-timeout`. Two design facts came from MEASUREMENT, not review: (a) a blocking no-reply `ping -c 2` costs **11.0s** on real hardware because GNU ping waits out a reply the workstation's firewall will never send — so the nudge is detached (~1.1s); (b) live testing showed the retry rung eating a whole 20s budget so peer-relay never ran — fixed with a budget reserve (`retry ≤ Budget/3`) + a 4s probe timeout, pinned by TestRetryRungCannotStarveThePeerRelay. Live-validated: local-prime correctly skips itself under WSL2 NAT, peer ranking picked the real peer, exit 1 on a host that stayed down (no false green). Tasks 18–23 done; 24 (hardware reproduction) needs elevation to flush the ARP entry. |
| 2026-08-16 | bulk-adopt | Operator: adopt all discovered hosts at once. Added `discover --add-all` (+ `--dry-run`, `--yes`) built on `sshconf.Mark`, accumulating into ONE config so the batch is a single write/backup instead of N. Verified live on a throwaway config: dry-run and decline mutate nothing, directives + an operator's own comment + pattern hosts survive, adopt-all→remove is byte-identical, and a second run writes nothing. 82 tests, coverage 80%. |
| 2026-08-16 | unattended | Operator: automate install.sh's password + y/n prompts for concurrent waves. Found both prompts already had unattended paths (gff override / --yes / --keep) and neither HANGS without a tty — the real blocker was sudo. Added: `WINSETUP_ANSWER` + `GEMINI_TEARDOWN_ANSWER` env contracts (verified by executing them, incl. flag-wins-over-env and unknown-value-warns), `runner.RunStdin` so the credential never touches argv/env, a pre-wave answer form (masked, empty each wave, esc discards), one-session prime + `sudo -n true` VERIFY with distinct exit codes 91/92. 79 tests, coverage 80%, shellcheck clean. |
| 2026-08-16 | build | Tasks 1-14 implemented in PR #227 (same-PR per operator). Split tui.go into model/view/keys/cmds; probeHost extracted so headless status is untouched. Concurrent background update engine + sudo-precheck fallback. 67 tests pass, `go vet` clean, **coverage 58.4% cmd / 80% fleet** (was 40.3/74). Two real defects caught by the new gates: lipgloss profile 0 is TrueColor not ASCII (goldens would have been colour-dependent), and a long remote failure log overflowed the row (now truncated). Demo: `FLEET_DEMO=1 go test ./cmd/ -run TestDemoFrames -v` (11 frames). Task 15 (live capture) still human-gated. |
| 2026-08-16 | mbo-amend | Operator: make sync + update concurrent in the TUI. Root cause named: ExecProcess suspends the whole TUI, so anything routed through it is inherently serial. Redesigned F8 as a background-first engine (≤ --jobs concurrent, BatchMode fast-FAIL, per-row log) with sudo-precheck routing to a serial interactive fallback (new F13); refresh works mid-wave excluding updating hosts (F2b); in-flight ownership invariant added. Plan now 15 tasks. |
| 2026-08-16 | mbo | Design + spec + plan + trio authored; worktree reset off a stale local main (hazard hit again — 3rd time); index.md registered; issue + draft PR pending. |

...
**Environment reality (explicit).** The dev host is **WSL/Linux**. AutoHotkey v2 runs only on the **Windows** side; `AutoHotkey.exe` lives at `/mnt/c/Users/<user>/AppData/Local/Microsoft/WindowsApps/AutoHotkey.exe` (Windows path `C:\Users\<user>\AppData\Local\Microsoft\WindowsApps\AutoHotkey.exe`). There is **no CI runner** for `.ahk` (the repo's `scripts/test.sh` does not reference AutoHotkey); `flow-calib-test.ahk` is run manually on the Windows host, signalling pass/fail via **exit code 0/1** plus an `.out` file in `%TEMP%`. Consequently:
...
"C:\Users\<user>\AppData\Local\Microsoft\WindowsApps\AutoHotkey.exe" flow-triggers-test.ahk ; echo EXIT=%ERRORLEVEL% ; type "%TEMP%\flow-triggers-test.out"
```

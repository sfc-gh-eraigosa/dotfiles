...
| `fleet-connect` | [design](./designs/fleet-connect.md) | [spec](./specs/fleet-connect.md) | [plan](./plans/fleet-connect.md) + [trio](./plans/fleet-connect/) | [#266](https://github.com/sfc-gh-eraigosa/dotfiles/issues/266) | design (this PR) | planning (provider **plugin framework**: versioned local JSON-RPC, MCP-shaped, `host/exec` callback onto the runner seam — plus TUI drill-down and the **herdr** provider as its first consumer; onboarding a tool is a `~/.config/fleet/providers.yaml` stanza. 25 tasks, 21 features, DAG in plan §6.1) |
| `fleet-connect-k8s` | — | — | — | — (blocked on `fleet-connect`) | — | idea (the second provider: contexts → namespaces → workloads → pods → containers, `kubectl logs -f` streams + `exec -it` handoffs; k3s on `<nano>`, kind on `<gigabyte>`/`<pi>`. Purely additive under `fleet-connect`'s protocol v1) |

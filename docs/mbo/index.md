...
| `fleet-config` | [design](./designs/fleet-config.md) | [spec](./specs/fleet-config.md) | [plan](./plans/fleet-config.md) + [trio](./plans/fleet-config/) | TBD | TBD | building (12 tasks + 4 human-evidenced gates; two strictly one-way verbs — `config pull` / `config push` — deliberately no bidirectional `sync`) |

...
| `gff-tui-vim` | — (bounded change to the existing `gff` TUI; design decided in-chat, recorded in the spec) | [spec](./specs/gff-tui-vim.md) | [plan](./plans/gff-tui-vim.md) + [trio](./plans/gff-tui-vim/) | (design issue pending anchor) | design worker `gff-tui-vim/…/design` (draft PR pending checkpoint) | planning (spec + plan approved 2026-09-05; build = 7 sequential TDD tasks on one `build` worker) |

...
| `sdk-tui` | [design](./designs/sdk-tui.md) + guide `sdk/libs/tui/GUIDE.md` | [spec](./specs/sdk-tui.md) | [plan](./plans/sdk-tui.md) + [trio](./plans/sdk-tui/) | [#283](https://github.com/sfc-gh-eraigosa/dotfiles/issues/283) | design [#286](https://github.com/sfc-gh-eraigosa/dotfiles/pull/286) · lib [#288](https://github.com/sfc-gh-eraigosa/dotfiles/pull/288) (merged into the design branch 2026-09-05 as `1821507`; #286 carries it to `main`, queued `ready-for-merge`) | in-review (shared TUI behaviors lib `sdk/libs/tui`: keymap · nav · prompt · search · cmdline · overlay; **blocks `gff-tui-vim`**; phase 3 = fleet port + gsl config studio) |
...
| `fleet-connect` | [design](./designs/fleet-connect.md) | [spec](./specs/fleet-connect.md) | [plan](./plans/fleet-connect.md) + [trio](./plans/fleet-connect/) | [#266](https://github.com/sfc-gh-eraigosa/dotfiles/issues/266) (sub-issues #289–#296, one per PR) | design [#267](https://github.com/sfc-gh-eraigosa/dotfiles/pull/267) · label `fleet-connect` | planning (provider **plugin framework**: versioned local JSON-RPC, MCP-shaped, `host/exec` callback onto the runner seam — plus TUI drill-down and the **herdr** provider as its first consumer; onboarding a tool is a `~/.config/fleet/providers.yaml` stanza; **port bridges** (2026-09-05): a `ports` provider and one `ssh -N` per host carrying N `-L`s, toggled with `t`/`T` or `fleet bridge`, never outliving fleet. 29 tasks, 26 features; **build split into eight stacked PRs, blocking-first** — plan §6.1) |
| `fleet-error-view` | [design](./designs/fleet-error-view.md) | [spec](./specs/fleet-error-view.md) | [plan](./plans/fleet-error-view.md) + [trio](./plans/fleet-error-view/) | [#308](https://github.com/sfc-gh-eraigosa/dotfiles/issues/308) | #TBD | planning (three-pane `fleet tui`: `h` hides the host list, `e` opens a **stderr** pane sharing the bottom with the log; one pane alone fills the viewport. stdout/stderr split at `runner.SplitStreamer` → `updexec.Console.ErrLine` → one tagged buffer with two projections; a non-benign stderr line puts `ok ⚠N` on the row **even when the run exits 0**. Also fixes a measured frame overflow of up to **+12 rows at 60×16**. 14 tasks, one PR — plan §6.1 records why it is not broken out) |
| `fleet-connect-k8s` | [design](./designs/fleet-connect-k8s.md) | [spec](./specs/fleet-connect-k8s.md) | [plan](./plans/fleet-connect-k8s.md) + [trio](./plans/fleet-connect-k8s/) | [#297](https://github.com/sfc-gh-eraigosa/dotfiles/issues/297) (sub-issues #298–#301) | design in [#267](https://github.com/sfc-gh-eraigosa/dotfiles/pull/267) | planning (priority 3 after herdr and ports: k3s on `<nano>`, kind on `<gigabyte>`/`<pi>`; five levels contexts → namespaces → kinds → objects → containers, `l d e` streams, `x` exec, `t` service tunnels kept alive by `kubectl port-forward` via `Tunnel.Keeper`; 8 tasks, four stacked PRs kA–kD after `fleet-connect` PR 3) |

...
| `fleet-update` | [design](./designs/fleet-update.md) | [spec](./specs/fleet-update.md) | [plan](./plans/fleet-update.md) + [trio](./plans/fleet-update/) | [#265](https://github.com/sfc-gh-eraigosa/dotfiles/issues/265) | design [#270](https://github.com/sfc-gh-eraigosa/dotfiles/pull/270) (draft) | planning (config-driven multi-repo update DAG for `fleet update`: `~/.config/fleet/fleet.yaml`, `sync`/`run`/`gh-auth` steps, per-repo `local: skip\|rescue\|carry` + branch restore, retries/backoff/timeouts, gff `fleet.update.{enabled,config}`) |

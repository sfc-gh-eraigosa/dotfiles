...
| `fleet-config` | [design](./designs/fleet-config.md) | [spec](./specs/fleet-config.md) | — | TBD | TBD | specifying (design Proposed, spec Draft; plan + trio not started. Two strictly one-way verbs — `config pull` / `config push` — deliberately no bidirectional `sync`) |

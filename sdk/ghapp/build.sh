...
if [ -z "$GO_BIN" ]; then
    echo "WARNING: 'go' not found; skipping ghapp build."
    exit 0
fi

# Version comes from the module's git release tag (sdk/ghapp/vX.Y.Z) —
...
VERSION="$(sdk_version "ghapp" "$SCRIPT_DIR")"
...
LDFLAGS="-X github.com/sfc-gh-eraigosa/dotfiles/sdk/ghapp/internal/version.Version=$VERSION \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/ghapp/internal/version.Commit=$COMMIT \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/ghapp/internal/version.BuildDate=$DATE \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/ghapp/internal/version.Dirty=$DIRTY"

mkdir -p "$BIN_DIR"
echo "Building ghapp v$VERSION with $($GO_BIN version)..."
cd "$SCRIPT_DIR"
"$GO_BIN" build -ldflags "$LDFLAGS" -o "$BIN_DIR/ghapp" main.go
echo "ghapp built and installed to $BIN_DIR/ghapp"

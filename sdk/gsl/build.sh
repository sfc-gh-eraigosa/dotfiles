...
if [ -z "$GO_BIN" ]; then
    echo "WARNING: 'go' not found; skipping gsl build."
    exit 0
fi

# Version comes from the module's git release tag (sdk/gsl/vX.Y.Z) —
...
VERSION="$(sdk_version "gsl" "$SCRIPT_DIR")"
...
LDFLAGS="-X github.com/sfc-gh-eraigosa/dotfiles/sdk/gsl/internal/version.Version=$VERSION \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/gsl/internal/version.Commit=$COMMIT \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/gsl/internal/version.BuildDate=$DATE \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/gsl/internal/version.Dirty=$DIRTY"

mkdir -p "$BIN_DIR"
echo "Building gsl v$VERSION with $($GO_BIN version)..."
cd "$SCRIPT_DIR"
"$GO_BIN" build -ldflags "$LDFLAGS" -o "$BIN_DIR/gsl" main.go
echo "gsl built and installed to $BIN_DIR/gsl"

# Dependency + seam gate.

...
if [ -z "$GO_BIN" ]; then
    echo "WARNING: 'go' not found; skipping gff build."
    exit 0
fi

# Version comes from the module's git release tag (sdk/gff/vX.Y.Z) —
...
VERSION="$(sdk_version "gff" "$SCRIPT_DIR")"
...
LDFLAGS="-X github.com/sfc-gh-eraigosa/dotfiles/sdk/gff/internal/version.Version=$VERSION \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/gff/internal/version.Commit=$COMMIT \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/gff/internal/version.BuildDate=$DATE \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/gff/internal/version.Dirty=$DIRTY"

mkdir -p "$BIN_DIR"
echo "Building gff v$VERSION with $($GO_BIN version)..."
cd "$SCRIPT_DIR"
"$GO_BIN" build -ldflags "$LDFLAGS" -o "$BIN_DIR/gff" main.go
echo "gff built and installed to $BIN_DIR/gff"

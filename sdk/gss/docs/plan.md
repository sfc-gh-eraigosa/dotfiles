...
- Reviewer personas come from
  `/Users/eraigosa/GitHub/<private-test-repo>/.agents/workflows/` — subagents
  adopt them by reading the relevant file first.
- The executor flips each `- [ ]` to `- [x]` as PRs merge into
  `test_gss` (in `~/GitHub/<private-test-repo>`). Final integration PR opens
...
- [ ] Every PR-NN above merged into `test_gss` in `~/GitHub/<private-test-repo>`.

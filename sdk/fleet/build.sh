...
if [ -z "$GO_BIN" ]; then
    echo "WARNING: 'go' not found; skipping fleet build."
    exit 0
fi

# Version comes from the module's git release tag (sdk/fleet/vX.Y.Z) —
...
VERSION="$(sdk_version "fleet" "$SCRIPT_DIR")"
...
cd "$SCRIPT_DIR"
"$GO_BIN" build -ldflags "$LDFLAGS" -o "$BIN_DIR/fleet" main.go
echo "fleet built and installed to $BIN_DIR/fleet"

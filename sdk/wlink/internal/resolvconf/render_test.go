...
// FOUND BY LIVE ACCEPTANCE (P14). A real machine's resolv.conf carried
// `search localdomain`, and pinning would have silently dropped it — changing
// how every unqualified name resolves. wlink owns which NAMESERVERS are used;
// search domains, sortlist, and any other directive belong to whoever set them.
func TestRenderResolvConf_PreservesSearchAndOtherDirectives(t *testing.T) {
	original := "" +
		"# This file was automatically generated by WSL.\n" +
		"nameserver 10.255.255.254\n" +
		"search localdomain example.internal\n" +
		"domain example.internal\n" +
		"sortlist 10.10.0.0/24\n"

	got := RenderResolvConf(Render{
		Winner:    "10.10.0.1",
		Fallbacks: []string{"10.255.255.254"},
		Preserve:  original,
	})
	...
	for _, want := range []string{
		...
		if !strings.Contains(got, want) {
			t.Errorf("dropped %q from the user's resolv.conf:\n%s", want, got)
		}
	}
	// Our own options line must win over any the original carried, and the
	// original's nameservers must not be duplicated verbatim.
	if strings.Count(got, "options ") != 1 {
		t.Errorf("expected exactly one options line:\n%s", got)
	}
	if strings.Count(got, "nameserver 10.255.255.254") != 1 {
		t.Errorf("fallback duplicated:\n%s", got)
	}
}

// An original that carries its own `options` must not produce two, and ours
// wins — the timeout tuning is the point of pinning.
func TestRenderResolvConf_OurOptionsWinOverTheOriginals(t *testing.T) {
	got := RenderResolvConf(Render{
		Winner:   "10.10.0.1",
		Preserve: "options timeout:5 attempts:5 ndots:2\nnameserver 10.255.255.254\n",
	})
	if !strings.Contains(got, "options timeout:1 attempts:1") {
		t.Errorf("our options line missing:\n%s", got)
	}
	if strings.Contains(got, "timeout:5") {
		t.Errorf("the original's timeout survived and would defeat the pin:\n%s", got)
	}
}

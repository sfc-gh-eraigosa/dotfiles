...
if [ -z "$GO_BIN" ]; then
    echo "WARNING: 'go' not found; skipping wlink build."
    exit 0
fi

# Version comes from the module's git release tag (sdk/wlink/vX.Y.Z) —
...
VERSION="$(sdk_version "wlink" "$SCRIPT_DIR")"
...
LDFLAGS="-X github.com/sfc-gh-eraigosa/dotfiles/sdk/wlink/cmd.Version=$VERSION \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/wlink/cmd.Commit=$COMMIT \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/wlink/cmd.BuildDate=$DATE \
         -X github.com/sfc-gh-eraigosa/dotfiles/sdk/wlink/cmd.Dirty=$DIRTY"

mkdir -p "$BIN_DIR"
echo "Building wlink v$VERSION with $($GO_BIN version)..."
cd "$SCRIPT_DIR"
"$GO_BIN" build -ldflags "$LDFLAGS" -o "$BIN_DIR/wlink" main.go
echo "wlink built and installed to $BIN_DIR/wlink"

...
#   * Write / Edit / MultiEdit / NotebookEdit / write_file / replace — scanned
#     ONLY when the target file is tracked-able (i.e. `git check-ignore` says it
#     is NOT ignored and it lives inside a git repo). Local-only files
#     (~/.claude, ~/.remember, memory notes, gitignored scratch) are skipped —
#     they never get published.
#   * Bash / run_shell_command — scanned ONLY for publishing verbs: `gh pr|issue
#     create|edit|comment`, `git commit`, `git tag -m`. Other Bash passes through.
#
# Contract:
...
case "$TOOL_NAME" in
    ...
    *)
        # Default allow for unknown tools (fail open)
        [[ "$TOOL_NAME" =~ ^[a-z_]+$ ]] && echo '{"decision": "allow"}'
        exit 0 ;;
esac

[ -z "$TEXT" ] && { [[ "$TOOL_NAME" =~ ^[a-z_]+$ ]] && echo '{"decision": "allow"}'; exit 0; }
...
if [ -n "$TARGET" ] && command -v git > /dev/null 2>&1; then
    tdir="$(dirname "$TARGET")"
    if git -C "$tdir" rev-parse --is-inside-work-tree > /dev/null 2>&1; then
        ...
        [[ "$TOOL_NAME" =~ ^[a-z_]+$ ]] && echo '{"decision": "allow"}'
        exit 0       # outside repo -> local only
    fi
fi
...
deny() {
    ...
    local reason="BLOCKED by privacy_guard: $category. Offending text: $snippet. Do not write local identity or secrets into committed/published content. Use a variable or placeholder instead: \$HOME / ~ / \${USER} / <user> / <host> / <REDACTED>."

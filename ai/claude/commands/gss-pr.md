...
4. After execution, surface the PR URL prominently, then ask whether to open it via the `open-url <url>` helper (`opt/scripts/misc/open-url`) — it picks the right opener per platform (macOS `open`, WSL `wslview`/`explorer.exe`, Linux `xdg-open`).

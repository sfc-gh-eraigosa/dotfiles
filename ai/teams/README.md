...
transforms each persona into a native agent for all three tools and degrades gracefully
...
The Ollama Modelfiles (`~/.config/ollama/teams/<team>/<role>.Modelfile`) are always
written. The installer runs `ollama create` only for agents whose base model is already
pulled, and it never pulls one itself, because a create against a missing base model
downloads it (up to 18 GB). For each missing base model it prints one line with the
command to fix it (`ollama pull <model>`). If a board can't hold the models, turn the
creates off and keep the Modelfiles with `gff set install.ai.teams-ollama-create false`.

Run **`/team <team\|squad>`** (Claude + Antigravity) to route a task to the right team and
...
4. Run `bash ai/teams/validate.sh` (consistency) and `bash ai/teams/gen-index.sh`
   (regenerates `INDEX.md`, per-team + root `AGENTS.md` + `CLAUDE.md` symlinks, and the
   `/team` command). Add the team to the table above.
5. `sync-teams` (or rerun `install.sh`) to emit the new agents for all three tools.

## Compatibility

Agents in this directory are compatible with:
- **Claude Code** — reads `claude:` model line + `Context-Strategy` for effort hints
- **Antigravity CLI** — reads `antigravity:` model line + `effort` hint

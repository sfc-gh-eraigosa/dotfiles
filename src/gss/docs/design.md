...
`src/gss/internal/identity/wordlist.txt`):
...
2. **Worker agents** (running *inside* a worker worktree, driving the
   actual code change) → `src/gss/skill/SKILL.md`.
...
#### `src/gss/skill/SKILL.md` — worker / power-user surface
...
`gss` lives under `src/gss/` in the dotfiles repo, alongside `tmux-mgr`,
...
|------------------|-------------------------------------------------------------------|-----------------------------|-------------------------------------------|
| `gss status`     | Show working-tree dirty files (`git status --porcelain`).         | `src/gss/cmd/status.go`     | `cmd/gss/status.go` + `internal/status/`  |
| `gss push`       | Backup branch → sync (rebase) → push → auto-PR on feature branch. **Approval-token gated**; `--force-autonomous` bypass retained. | `src/gss/cmd/push.go`       | `cmd/gss/push.go` + `internal/classic/push.go` (uses `internal/approval`, `internal/backup`, `internal/sync`, `internal/gh`) |
| `gss sync`       | Fetch origin, `git pull --rebase`, surface conflicts.             | `src/gss/cmd/sync.go`       | `cmd/gss/sync.go` + `internal/sync/`      |
| `gss backup`     | Create `backup/gss-TIMESTAMP` local safety branch.                | `src/gss/cmd/backup.go`     | `cmd/gss/backup.go` + `internal/backup/`  |
| `gss scan [dir]` | Walk filesystem, list `[DIRTY]` repos via `git status --porcelain`. | `src/gss/cmd/scan.go`     | `cmd/gss/scan.go` + `internal/scan/`      |
| `gss pr`         | If on default branch, create `feature/gss-YYYYMMDD-HHMMSS` + push + open PR. **Approval-token gated**. | `src/gss/cmd/pr.go`     | `cmd/gss/pr.go` + `internal/classic/pr.go` |
| `gss diff`       | Show staged + unstaged diff.                                      | `src/gss/cmd/diff.go`       | `cmd/gss/diff.go`                          |
| `gss version`    | Print version / commit / dirty / build date; `--json` supported.  | `src/gss/cmd/version.go`    | `cmd/gss/version.go` + `internal/version/` |
...
`src/gss/skill/SKILL.md` is updated only to add the new `feature`
verbs; classic behaviour stays as documented.

**Build / distribution** is preserved:
- `src/gss/VERSION` stays as the version source-of-truth.
- `src/gss/build.sh` still produces the binary; the new layout updates
...
> suite at every step.
>
> **Source of truth**: `/Users/eraigosa/git/dotfiles/src/gss/docs/design.md`.
...
>
> Author `/Users/eraigosa/git/dotfiles/src/gss/docs/plan.md`. It must:
>

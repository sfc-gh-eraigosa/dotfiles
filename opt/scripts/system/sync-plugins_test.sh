...
assert_contains "$LOCKOUT" "crashes on claude's partial-clone checkout and leaves the lock behind" "…and the warning names the real cause (a git that crashes), not a transient lock"
assert_contains "$LOCKOUT" "setup_git_apt_repo.sh" "…and it names the fix: a current git from the git-core PPA"
assert_contains "$LOCKOUT" "on this host git is " "…and it reports which git this host is running"
assert_not_contains "$LOCKOUT" "transient" "…and it never calls the lock transient"

...
- `sync-mcp.sh`: **Ensure-only sync of standalone MCP servers** from the `ai/mcp.yaml` manifest (a *separate concern* from plugins). Registers the listed servers at **user scope** for Claude (`claude mcp add`) + Gemini (`gemini mcp add`); never removes anything; idempotent (tolerates "already exists"). stdio transport only; **registration only — never runs the interactive browser auth**. Pinned versions, never `@latest`. Pass `--dry-run` to preview (works even with the CLIs absent). Mirrors `sync-plugins.sh`'s setsid/timeout hardening. First server: `notebooklm` (NotebookLM). Parsed with mikefarah `yq`. Companion test: `sync-mcp_test.sh`. Human docs: `docs/ai-mcp.md`.
